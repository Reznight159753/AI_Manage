...
function ModelDesign() {
  ...
  async function callSpeechToText(audioBlob) {
    ...
    const endpoints = [
      'https://7c8869980fc4.ngrok-free.app/transcribe',
  ...
  async function startListening() {
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Nếu vẫn trong voice chat mode và không bị user tắt thì tiếp tục lắng nghe
        if (isVoiceChatMode && !isProcessing) {
          setTimeout(() => {
            if (isVoiceChatMode) {
              startNewRecording(stream);
            }
          }, 1000); // Đợi 1s trước khi bắt đầu lắng nghe tiếp
        } else {
          stream.getTracks().forEach(track => track.stop());
        }
      };
      ...
      // Auto stop sau 10 giây để tránh recording quá dài
      setTimeout(() => {
  ...
  // Hàm helper để bắt đầu recording mới với stream hiện tại
  function startNewRecording(stream) {
    ...
    recorder.onstop = async () => {
      ...
      // Tiếp tục lắng nghe nếu vẫn trong voice chat mode
      if (isVoiceChatMode && !isProcessing) {
        setTimeout(() => {
          if (isVoiceChatMode) {
            startNewRecording(stream);
          ...
          <div className="input-container">
            ...
            <div className="button-group">
              ...
              <button
                ...
                className={`voice-button ${isVoiceChatMode ? 'voice-active' : ''} ${isListening ? 'listening' : ''}`}
                disabled={isProcessing}
                title={isVoiceChatMode ? "Tắt voice chat" : "Bật voice chat"}
              >
                {isVoiceChatMode ? 
                  (isListening ? <Mic size={16} className="pulse" /> : <Mic size={16} />) : 
                  <MicOff size={16} />
                }
              </button>
            </div>
